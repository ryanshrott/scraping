...
class Basic(scrapy.Spider):
    ...
    def start_requests(self):
        cities = ['toronto']
        urls = [f'https://www.airbnb.ca/s/{city}/homes/' for city in cities]
        print('start')
        for url in urls:
            print(url)
            yield scrapy.Request(url=url, callback=self.parse_item, headers={'User-Agent': "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.1 (KHTML, like Gecko) Chrome/22.0.1207.1 Safari/537.1"})
    ...
    def parse_item(self, response):
        ...
        hotels = response.css('div._8ssblpx ::attr(href)')
